...
function parseDestinations(destinations: string): string[] {
...
async function main() {
  const excelPath = process.argv[2] || '/Users/kevinjohn/Downloads/lh profiles 2.15.26 ok to load.xlsx';
  ...
  function findMatchingProfile(excelName: string, excelOrig: string): ProfileInfo | null {
    ...
    // 2. Extract segments and try to match using Orig column
    const segments = extractSegments(nameUpper);

    // 3. If Orig is in segments, find the next segment for the route
    ...
    // 4. If Orig is NOT in segments (e.g., 475MSPA with Orig=DUL),
    //    try Orig -> first segment route
    ...
    // 5. Fallback: try first two segments
  ...
  // Clear existing data
  const deleted = await prisma.profileOkayToLoad.deleteMany({});
  ...
  for (let i = 0; i < recordsArray.length; i += batchSize) {
    ...
    await prisma.profileOkayToLoad.createMany({
      data: batch,
      skipDuplicates: true
    });
