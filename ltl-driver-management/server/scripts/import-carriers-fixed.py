...
def main():
    ...
    # Insert new carriers one by one with individual transactions
    ...
    for index, row in df.iterrows():
        try:
            ...
            # Check for existing MC or DOT numbers to avoid conflicts
            if mc_number:
                cursor.execute("SELECT id FROM carriers WHERE \"mcNumber\" = %s", (mc_number,))
                if cursor.fetchone():
                    continue  # Skip duplicates
                    
            if dot_number:
                cursor.execute("SELECT id FROM carriers WHERE \"dotNumber\" = %s", (dot_number,))
                if cursor.fetchone():
                    continue  # Skip duplicates
            ...
            failed_inserts += 1
            if failed_inserts < 10:  # Only show first 10 errors
