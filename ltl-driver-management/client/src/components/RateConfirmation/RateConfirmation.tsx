            ...
            <div className="p-1">
              <div className="grid grid-cols-2 gap-1">
                <div>
                  ...
                  <div className="text-xs"><span className="font-bold">Address:</span> {childBooking.originAddress || childBooking.route?.originAddress || getLocationData(childBooking.route?.origin || childBooking.origin)?.address || '985 Glendale Avenue'}</div>
                  <div className="text-xs"><span className="font-bold">City, State Zip:</span> {childBooking.originCity || childBooking.route?.originCity || getLocationData(childBooking.route?.origin || childBooking.origin)?.city || 'SPARKS'}, {childBooking.originState || childBooking.route?.originState || getLocationData(childBooking.route?.origin || childBooking.origin)?.state || 'NV'} {childBooking.originZipCode || childBooking.route?.originZipCode || getLocationData(childBooking.route?.origin || childBooking.origin)?.zipCode || '89431'}</div>
                  <div className="text-xs"><span className="font-bold">Phone:</span> {getLocationData(childBooking.route?.origin || childBooking.origin)?.phone || '(775) 331-2311'} <span className="font-bold">Contact:</span> {childBooking.originContact || childBooking.route?.originContact || getLocationData(childBooking.route?.origin || childBooking.origin)?.contact || 'Brian Smith'}</div>
                  <div className="text-xs"><span className="font-bold">Hours:</span> {getLocationData(childBooking.route?.origin || childBooking.origin)?.hours || '04:00 -to-10:00'}</div>
                ...
                <div>
                  ...
                  <div className="text-xs"><span className="font-bold">Address:</span> {childBooking.destinationAddress || childBooking.route?.destinationAddress || getLocationData(childBooking.route?.destination || childBooking.destination)?.address || '2800 S El Dorado ST'}</div>
                  <div className="text-xs"><span className="font-bold">City, State Zip:</span> {childBooking.destinationCity || childBooking.route?.destinationCity || getLocationData(childBooking.route?.destination || childBooking.destination)?.city || 'STOCKTON'}, {childBooking.destinationState || childBooking.route?.destinationState || getLocationData(childBooking.route?.destination || childBooking.destination)?.state || 'CA'} {childBooking.destinationZipCode || childBooking.route?.destinationZipCode || getLocationData(childBooking.route?.destination || childBooking.destination)?.zipCode || '95206'}</div>
                  <div className="text-xs"><span className="font-bold">Phone:</span> {getLocationData(childBooking.route?.destination || childBooking.destination)?.phone || ''} <span className="font-bold">Contact:</span> {childBooking.destinationContact || childBooking.route?.destinationContact || getLocationData(childBooking.route?.destination || childBooking.destination)?.contact || ''}</div>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="p-1">
              <div className="grid grid-cols-2 gap-1">
                <div>
                  ...
                  <div className="text-xs"><span className="font-bold">Address:</span> {getLocationData(leg.origin)?.address || booking.originAddress || booking.route?.originAddress || '985 Glendale Avenue'}</div>
                  <div className="text-xs"><span className="font-bold">City, State Zip:</span> {getLocationData(leg.origin)?.city || booking.originCity || booking.route?.originCity || 'SPARKS'}, {getLocationData(leg.origin)?.state || booking.originState || booking.route?.originState || 'NV'} {getLocationData(leg.origin)?.zipCode || booking.originZipCode || booking.route?.originZipCode || '89431'}</div>
                  <div className="text-xs"><span className="font-bold">Phone:</span> {getLocationData(leg.origin)?.phone || '(775) 331-2311'} <span className="font-bold">Contact:</span> {getLocationData(leg.origin)?.contact || booking.originContact || booking.route?.originContact || 'Brian Smith'}</div>
                ...
                <div>
                  ...
                  <div className="text-xs"><span className="font-bold">Address:</span> {getLocationData(leg.destination)?.address || booking.destinationAddress || booking.route?.destinationAddress || '2800 S El Dorado ST'}</div>
                  <div className="text-xs"><span className="font-bold">City, State Zip:</span> {getLocationData(leg.destination)?.city || booking.destinationCity || booking.route?.destinationCity || 'STOCKTON'}, {getLocationData(leg.destination)?.state || booking.destinationState || booking.route?.destinationState || 'CA'} {getLocationData(leg.destination)?.zipCode || booking.destinationZipCode || booking.route?.destinationZipCode || '95206'}</div>
                  <div className="text-xs"><span className="font-bold">Phone:</span> {getLocationData(leg.destination)?.phone || ''} <span className="font-bold">Contact:</span> {getLocationData(leg.destination)?.contact || booking.destinationContact || booking.route?.destinationContact || ''}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-1">
            <div className="grid grid-cols-2 gap-1">
              <div>
                ...
                <div className="text-xs"><span className="font-bold">Address:</span> {getLocationData(booking.origin || booking.route?.origin)?.address || booking.originAddress || booking.route?.originAddress || '985 Glendale Avenue'}</div>
                <div className="text-xs"><span className="font-bold">City, State Zip:</span> {getLocationData(booking.origin || booking.route?.origin)?.city || booking.originCity || booking.route?.originCity || 'SPARKS'}, {getLocationData(booking.origin || booking.route?.origin)?.state || booking.originState || booking.route?.originState || 'NV'} {getLocationData(booking.origin || booking.route?.origin)?.zipCode || booking.originZipCode || booking.route?.originZipCode || '89431'}</div>
                <div className="text-xs"><span className="font-bold">Phone:</span> {getLocationData(booking.origin || booking.route?.origin)?.phone || '(775) 331-2311'} <span className="font-bold">Contact:</span> {getLocationData(booking.origin || booking.route?.origin)?.contact || booking.originContact || booking.route?.originContact || 'Brian Smith'}</div>
              ...
              <div>
                ...
                <div className="text-xs"><span className="font-bold">Address:</span> {getLocationData(booking.destination || booking.route?.destination)?.address || booking.destinationAddress || booking.route?.destinationAddress || '2800 S El Dorado ST'}</div>
                <div className="text-xs"><span className="font-bold">City, State Zip:</span> {getLocationData(booking.destination || booking.route?.destination)?.city || booking.destinationCity || booking.route?.destinationCity || 'STOCKTON'}, {getLocationData(booking.destination || booking.route?.destination)?.state || booking.destinationState || booking.route?.destinationState || 'CA'} {getLocationData(booking.destination || booking.route?.destination)?.zipCode || booking.destinationZipCode || booking.route?.destinationZipCode || '95206'}</div>
                <div className="text-xs"><span className="font-bold">Phone:</span> {getLocationData(booking.destination || booking.route?.destination)?.phone || ''} <span className="font-bold">Contact:</span> {getLocationData(booking.destination || booking.route?.destination)?.contact || booking.destinationContact || booking.route?.destinationContact || ''}</div>
              </div>
            </div>
          </div>
        </div>
